...
then
    root -b -q $PROMPT_DIR'/beam-mod/scripts/japan_plot_beammod_BPMS_cyc_12X.C("'$QW_ROOTFILES'",'$run_num')';
fi
if  [ $(($run_num)) -ge  3404 ] && [ $(($run_num)) -le  4980 ]
then
    root -b -q $PROMPT_DIR'/beam-mod/scripts/japan_plot_beammod_BPMS_cyc_prompt.C("'$QW_ROOTFILES'", '$run_num')';
fi

root -b -q $PROMPT_DIR'/beam-mod/scripts/japan_plot_beammod_quartz_cyc_prompt.C("'$QW_ROOTFILES'", '$run_num')';

if  [ $(($run_num)) -ge  3803 ]
then
    root -b -q $PROMPT_DIR'/beam-mod/scripts/japan_plot_beammod_at_cyc_prompt.C("'$QW_ROOTFILES'", '$run_num')';
fi
