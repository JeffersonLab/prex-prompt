...
def main():
    ...
    #runfile=open(_source+"/prex-runlist/crex-respin2/simple_list/all_crex.list","r") # Skip AT, handled separately
    runfile=open(_source+"/prex-runlist/crex-respin2/simple_list/all_nonAT_crex.list","r") # Skip AT, handled separately
    for line in runfile:
        if (len(line) < 4):
            continue
