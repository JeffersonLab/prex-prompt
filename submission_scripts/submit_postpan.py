...
def main():
    ...
    if submit==1:
        if useSWIF==1:
            print "Submitting postpan prompt analysis for runs "+str(firstrun)+" to "+str(lastrun)+" using designated SWIF workflow "+str(_workflowID)
...
def createXMLfile(mssdir,source,rootout,nStart,nStop,email,workflowID,runlist):
    ...
    for nr in runlist:
        ...
        f.write("    "+source+"/overload_postpan.sh "+str(nr)+"; \n")
        f.write("    ]]></Command>\n")
        f.write("    <Stdout dest=\""+source+"/LogFiles/postpan_ifarmlog"+"_%04d"%(nr)+".out\"/>\n")
        f.write("    <Stderr dest=\""+source+"/LogFiles/postpan_ifarmlog"+"_%04d"%(nr)+".err\"/>\n")
        f.write("  </Job>\n\n")
