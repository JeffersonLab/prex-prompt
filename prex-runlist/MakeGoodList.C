...
void MakeGoodList(TString prefix = "crex-respin2/"){
  ...
  FILE* all_out = fopen(Form("%sgood_list/all.list",prefix.Data()),"w");
  auto iter_slug = fSlugArray.begin();
  while(iter_slug!=fSlugArray.end()){
    Int_t mySlug = (*iter_slug).first;
    TString new_filename = Form("%sgood_list/slug%d.list",prefix.Data(),mySlug);
    FILE* output;
