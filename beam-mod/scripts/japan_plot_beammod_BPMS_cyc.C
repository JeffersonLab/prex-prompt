...
int japan_plot_beammod_BPMS_cyc(int runNo=0) { 
  ...
  for(int i=0;i<n;i++){
	...
	int ndata = tree_R->Draw(Form("%lf*%s:(%s*%lf)",
				 ...
				 Form("bcm_dg_ds>65 && bmod_ramp>0 && bmwobj==%d && abs(%s-%f)>20 && bmwcycnum==%f",
	...
	if(ndata>50){
	  ...
	  if(sens_err[ibpm][icoil]==-1){
	    ...
	    double last_error = sens_err[ibpm][icoil];
	    double last_slope = sens[ibpm][icoil];
	    double weight = 1.0/pow(last_error,2)+1.0/pow(this_error,2);
	    sens[ibpm][icoil]=(1.0/weight)*(last_slope*1.0/pow(last_error,2)+
					    this_slope*1.0/pow(this_error,2));
	    sens_err[ibpm][icoil]=last_error*this_error/sqrt(pow(last_error,2)
							     +pow(this_error,2));
	  }
	}
  ...
  ostringstream sstr0;
  sstr0<<"./dit_txt/BPMs_sensitivity_run"<<runNo<<".txt";
