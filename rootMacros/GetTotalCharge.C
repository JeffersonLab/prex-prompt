void GetTotalCharge(Int_t runNum){
  
//  FILE *capacitor_txt = fopen("capacitor.log","a");
  TFile *rootfile = TFile::Open(Form("$QW_ROOTFILES/prexPrompt_pass1_%d.000.root",runNum));
  if (!rootfile) {
    cout << "0.0" << endl;
    return;
  }
...
//  TString seg = run_dot_seg(run_dot_seg.First('.')+1,3);
  ...
  TString bcm_name = "yield_bcm_an_us";
  
  TCut goodPatternCut = "ErrorFlag==0 && yield_bcm_an_us.Device_Error_Code==0";
//  Int_t nGoodPatterns = mul_tree->GetEntries(goodPatternCut);
  Int_t nTotalPatterns = mul_tree->GetEntries();
...
//  Double_t current_mean = h_buff->GetMean();
  mul_tree->Draw("yield_bcm_an_us","","goff");
  TH1D* h_buff_total = (TH1D*)gDirectory->FindObject("htemp");
  Double_t current_mean = h_buff_total->GetMean();
  
//  Double_t integral_raw = current_mean*nGoodPatterns;
//  Double_t all_integral_raw = current_mean*nTotalPatterns;

//  Double_t integration_time = 1.0/30.0; //4*8.2e-3; //sec // FIXME hardcoded to 30 Hz (mul patterns should be)...
//  Double_t uA = 1e-6; // Amp

//  Double_t charge_in_C = integral_raw*integration_time*uA;

//  cout << "--nPatterns: " << nGoodPatterns << endl;
  cout << current_mean << endl;
