// CheckNormalizedAT.C : 
//	ROOT script for checking AT Detector signal from event and mul tree
//
...
void CheckNormalizedAT(){
  ...
  TGraph* g_buff;
  evt_tree->SetAlias("CurMon","(bcm_an_us>2.5 ? bcm_an_us : 1)");
  for(int idet=0;idet<ndet;idet++){
    const char* device_name = vAT[idet];
    ...
    plot_title = Form("run%s_at_%s.png",
		      run_seg.Data(),
		      device_name);
