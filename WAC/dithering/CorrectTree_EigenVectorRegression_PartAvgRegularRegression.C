...
using namespace std;
void CorrectTree_EigenVectorRegression_PartAvgRegularRegression(Int_t run_number=0, std::string stub="" ){
  ...
  // segment averaged analysis : TFile* eigenvectorComboOutput = TFile::Open(Form("%s/rcdb_segment_eigenvectors_sorted_seg_slug_avg.root",ditSlopeFileNamebase.Data())); // FIXME needs to be generalized eventually - stub.c_str()
  TFile* eigenvectorRegressionSlopesOutput = TFile::Open(Form("%s/rcdb_segment_parts_eigenvectors_sorted.root",ditSlopeFileNamebase.Data())); // FIXME needs to be generalized eventually - stub.c_str()
  TFile* eigenvectorComboOutput = TFile::Open(Form("/lustre19/expphy/volatile/halla/parity/crex-respin1/LagrangeOutput/rootfiles/rcdb_segment_eigenvectors_sorted_part_seg_slug_avg.root")); // FIXME needs to be generalized eventually - stub.c_str()

  if(eigenvectorComboOutput==NULL){
    std::cout << "Error: " 
      << "dit RootFile doesn't exist!" << std::endl;
    return;
  }

  if(eigenvectorRegressionSlopesOutput==NULL){
    std::cout << "Error: " 
      << "dit RootFile doesn't exist!" << std::endl;
    return;
  }

  TTree *mini_tree = (TTree*)eigenvectorRegressionSlopesOutput->Get("mini");
  TString combo_tree_name = "mini_eigen_reg_5bpms_sorted_part_avg";
  TString eig_slope_tree_name = "mini_eigen_reg_5bpms_part_avg";
  TTree *combo_tree = (TTree*)eigenvectorComboOutput->Get(combo_tree_name); // FIXME hardcoded to the segment average eigenvector analysis
  TTree *eig_slope_tree = (TTree*)eigenvectorRegressionSlopesOutput->Get(eig_slope_tree_name); // FIXME hardcoded to the segment average eigenvector analysis
  mini_tree->AddFriend(combo_tree);
  mini_tree->AddFriend(eig_slope_tree);

  if(combo_tree==NULL || eig_slope_tree==NULL || mini_tree==NULL){
    std::cout << "Error: " 
  ...
  for(int idet = 0; idet<eigen_reg_ndet; idet++){
    tmp_slopes.clear();
    for(int imon = 0; imon <nmon; imon++){
      slope_struct tmp_slope;
      tmp_slopes.push_back(tmp_slope);
    }
    Printf("Test 1");
    eig_slope_matrix.push_back(tmp_slopes);
  }
  for(int idet = 0; idet<eigen_reg_ndet; idet++){
    for(int imon = 0; imon <nmon; imon++){
      TString slope_varname = "";
      slope_varname = Form("%s.%s_%s",eig_slope_tree_name.Data(),eigen_reg_maindet_array[idet].Data(),bpm_array[imon].Data());
